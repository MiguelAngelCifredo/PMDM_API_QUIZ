...
public class QuizActivity extends AppCompatActivity implements QuestionFragment.OnAnswerSelectedListener {
    ...
    private void loadQuestions() {
        ...
        api.getCuestionario(idunit, 10, "s").enqueue(new Callback<List<Question>>() {
            @Override
            public void onResponse(@NonNull Call<List<Question>> call, @NonNull Response<List<Question>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    questionsList = response.body();
                    if (questionsList.isEmpty()) {
                        Toast.makeText(QuizActivity.this, "No hay preguntas", Toast.LENGTH_SHORT).show();
                        finish();
                        return;
                    }
            ...
            public void onFailure(@NonNull Call<List<Question>> call, @NonNull Throwable t) {
                Toast.makeText(QuizActivity.this, "Error de conexión", Toast.LENGTH_SHORT).show();
            }
        });
    }
